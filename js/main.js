...
(function() {
    ...
    function changeHospitalList (d) {
        ...
        if (addOrHideData){
            var addItem = hideItems.filter(addHideItem)[0];
            selectedData.items.push(addItem);
            hideItems = hideItems.filter(updateHideItem);

            selectedData.items = selectedData.values.filter(function(item, i){
                if (selectedData.items.indexOf(item) !== -1){
                    return item
                }
            });
        }else{
            selectedData.items = selectedData.items.filter(hideItem);
        }
